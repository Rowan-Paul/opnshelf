...
Accounts created on our Tranquil PDS cannot write records until a notification channel (email) is verified — the PDS raises `AccountNotVerified` ("You must verify at least one notification channel before creating records"). Because opnshelf eagerly seeded a profile record + default lists the instant an account was born, every native signup failed that write. We chose to **keep the gate on** and split signup into create-account → verify-email → Core OAuth handoff → seed records and start onboarding, rather than flip the gate off.
...
- New backend `POST /auth/verify-email { code }`: restores the caller's short-lived bootstrap credential session, reads the email via `getSession`, and calls `com.atproto.server.confirmEmail({ email, token: code })`. It performs no repository writes. Once verification succeeds, opnshelf revokes the unrestricted bootstrap session before redirecting through Core OAuth; cancelling that authorization therefore leaves the account verified but signed out. Only after the scoped OAuth session succeeds does opnshelf seed the profile + default lists. `POST /auth/resend-verification` → Tranquil's `com.atproto.server.resendVerification` (DID-keyed, unauthenticated; Tranquil does not implement the standard `requestEmailConfirmation`).
