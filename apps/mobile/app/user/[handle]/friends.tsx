...
export default function FriendsScreen() {
	...
	const searchQuery = useInfiniteQuery({
		...
		enabled: Boolean(isAuthenticated && user?.did && isOwner && hasSearch),
		initialPageParam: 1,
	...
	const followingQuery = useInfiniteQuery({
		...
		enabled: Boolean(
			user?.did && handle && isOwner && activeTab === "following",
		),
		initialPageParam: 1,
		getNextPageParam: (lastPage) =>
			lastPage.hasNextPage ? lastPage.page + 1 : undefined,
	...
	const followersQuery = useInfiniteQuery({
		...
		enabled: Boolean(
			user?.did && handle && isOwner && activeTab === "followers",
		),
		initialPageParam: 1,
		getNextPageParam: (lastPage) =>
			lastPage.hasNextPage ? lastPage.page + 1 : undefined,
			...
			>
				<View style={styles.centerContent}>
					...
					>
						<CardHeader>
							<Text style={[styles.stateTitle, { color: colors.onSurface }]}>
								Friends is only available on your profile
							</Text>
							<Text
								style={[
									styles.stateDescription,
							...
							>
								Use the public Followers and Following tabs when viewing other
								people.
							</Text>
						</CardHeader>
					</Card>
