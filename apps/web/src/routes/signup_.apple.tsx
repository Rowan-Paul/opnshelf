import { authControllerAppleRegisterMutation } from "@opnshelf/api";
...
function AppleSignupPage() {
	...
	// empty and Onboarding collects the display name later (ADR 0027).
	...
	const registerMutation = useMutation({
		mutationKey: ["auth", "apple-register"],
		...authControllerAppleRegisterMutation(),
		onSuccess: (data) => {
	...
	useEffect(() => {
		fetch(`${env.VITE_API_URL}/auth/apple/pending`, {
			credentials: "include",
			cache: "no-store",
		})
			.then(async (response) => {
	...
	// No pending Apple signup to finish (a bookmarked or reloaded URL).
	if (!email) {
		return (
